---
title: "FY2024 - Preparing New Fiscal Data for Fiscal Futures Project"
...
rev_fy24 <- read_xlsx("data/revenue/Fis_Fut_Rev_071124.xlsx") %>% 
  rename(fy = 'FISCAL YEAR',
         fund = 'FUND',
         fund_name = 'FUND NAME',
         agency = 'AGENCY',
         ...
         receipts = 'AMOUNT'
  )  %>% mutate(data_source = "rev IOC 07.11.2024")

exp_fy24 <- read_xlsx("data/expenditures/Fis_Fut_Exp_071124.xlsx") %>% 
  rename(fy = 'FISCALYEAR',
         fund = 'FUND',
         fund_name = 'FUND NAME',
         agency = 'AGENCY',
         agency_name = 'AGENCY NAME',
         ...
         appn_net_xfer = 'NET OF TRANS AMT',
         expenditure = 'EXPENDED THRU 6/30') %>%
  mutate( data_source = "exp IOC 07.11.2024"
  ) 
...
The code chunk below reads in the previosly combined .dta files as one csv file and allows the new data to be combined with `rbind`. Previous year's data cleaning files show the code for combinging the .dta files for all fiscal years before FY 2022. Variable names were manually changed by past researchers so that they were consistent across years.
...
```{r create-rev-csv-FY24, eval=FALSE}



allrevfiles23 <- read_csv("data/revenue/allrevfiles_Nov132023.csv")

# add in excel file for current year
allrevfiles24 <- allrevfiles23 %>% 
...
allexpfiles23 <- read_csv("data/expenditures/allexpfiles_Nov142023.csv")

# add in excel file for current year
allexpfiles24 <- allexpfiles23 %>% 
...
write_csv(allrevfiles24, "data/allrevfiles_Sept92024.csv")


write_csv(allexpfiles24, "data/allexpfiles_Sept92024.csv")
...
allrevfiles24 <- read_csv("./data/allrevfiles_Sept92024.csv") #combined but not recoded
allexpfiles24 <- read_csv("./data/allexpfiles_Sept92024.csv") #combined but not recoded
...
anti_join(agencies24, agencies_past, 
...
anti_join(orgs24, orgs_past,
          by = c("appr_org", "org_name")) %>% 
  arrange(appr_org)
```

11 funds identified from the expenditure file as having something changed compared to funds that existed in previous years. They are either new funds, reused funds, or have very minor typos or changes in their name. Some are also identified in the revenue section below:
...
# agencies24 # 81 agencies this year
agencies24 <- allrevfiles24  %>% 
  filter(fy == 2024) %>% 
  mutate(agency == as.character(agency)) %>% 
  group_by(agency, agency_name) %>% 
  summarize(Receipts = sum(receipts, na.rm = TRUE))
...
anti_join(agencies24, agencies_past, by = c("agency", "agency_name")) %>% 
  arrange(agency)


funds_past <- allrevfiles24  %>% 
  filter(fy < 2024) %>% 
  mutate(fund == as.character(fund)) %>% 
  group_by(fund, fund_name) %>% 
...
# 0 revenue funds were in FY24 revenue data that were not in past data 
anti_join(funds24, funds_past, by = c("fund", "fund_name")) %>% 
  arrange(fund)



sources_past <- allrevfiles24  %>% 
  filter(fy < 2024) %>% 
...
#  revenue sources were in FY24 data that were not in past data 
# some could be small source name changes:
anti_join(sources24, sources_past, by = c("source", "source_name")) %>% 
  arrange(source)
...
```{r create-exp_temp}
library(readxl)
funds_ab_in_2024 = readxl::read_excel("C:/Users/aleaw/OneDrive/Documents/PhD Fall 2021 - Spring 2022/Merriman RA/Fiscal Futures IGPA/Fiscal-Future-Topics/inputs/funds_ab_in_2024.xlsx")


exp_temp <- exp_1998_2024 %>% 
  arrange(fund, fy) %>%
  filter(expenditure != 0) %>%             # keeps everything that is not zero
# join  funds_ab_in_2021  to exp_temp
 left_join(funds_ab_in_2024, by = "fund") %>% # matches most recent fund number 
...
rev_temp <- inner_join(rev_1998_2024, funds_ab_in_2024, by = "fund") %>% arrange(source)

# need to update the ioc_source_type file every year! 
ioc_source_type = readxl::read_excel("C:/Users/aleaw/OneDrive/Documents/PhD Fall 2021 - Spring 2022/Merriman RA/Fiscal Futures IGPA/Fiscal-Future-Topics/inputs/ioc_source_updated24_AWM.xlsx")
...
pension_check3 %>% group_by(fy) %>% 
  ...
  geom_line() + 
    theme_classic()+
  labs (title = "Pension Benefits Paid to Retired Employees")
...
pension_picture <- exp_temp %>% 
  ...
  #object == "4430" & fund == "0825" ~ "Pension Buyout/Benefits Paid Early; INCLUDED",
    ...
    (object=="4431") ~ "State Pension Contributions", # 4431 = state payments into pension fund
        (obj_seq_type > "11590000" & obj_seq_type < "11660000")  ~ "IOC Retirement Expense Object",
    # objects 1159 to 1166 are all considered Retirement by Comptroller 
...
employer_contributions <- exp_temp %>% 
  filter(object == "1180") %>% group_by(fy) %>% summarize(object1180 = sum(expenditure)) 
...
> changing pensionscost. keep in funtion of spending
...
write_csv(exp_temp, "data/exp_temp.csv")
...
write_csv(exp_long, "expenditures_recoded_long_FY24.csv")
write_csv(rev_long, "revenue_recoded_long_FY24.csv")
...
exp_long %>%
 ...
 # select(-c(Year, `Total Expenditures`)) %>%
  ...
  # geom_col(aes(x = fct_reorder(Category_name, `Dollars`), y = `Dollars`, fill = "red"))+ 
    geom_col(aes(x = Category_name, y = `Dollars`, fill = "red"))+ 
...
exp_long %>%
 ...
 # geom_col(aes(x = fct_reorder(Category_name, `Dollars`), y = `Dollars`), fill = "rosybrown2")+ 
    geom_col(aes(x = Category_name, y = `Dollars`), fill = "rosybrown2")+ 
...
**Changes in Categories - 2023 to 2024**:
...
rev_long %>%
  ...
  labs(title = "2023 to 2024 Change in Revenue", x = "Millions of Dollars" , y = "",  caption = "")  +
...
exp_long %>%
  ...
  geom_text(aes(x = ifelse(Year == current_year, as.numeric(Dollars), NA),  
            ...
            size = 2.8) +
  geom_point(aes(color = Year), size=2)  +
  labs(title = "2023 to 2024 Change in Expenditures", x = "Millions of Dollars" , y = "",  caption = "")  +
  scale_fill_manual(values = c("#d62828", "#003049"), labels = c("FY 2023", "FY 2024"))+
  scale_color_manual(values = c("#d62828", "#003049")) +
  theme_classic()+ 
  theme(
...
cagr_25 <- calc_cagr(exp_long, num_years) %>% 
  # group_by(Category) %>%
...
CAGR_expenditures_summary <- data.frame(cagr_1, cagr_2, cagr_3, cagr_5, cagr_10, cagr_25 ) %>% 
  select(-c(Category_name.1, Category_name.2, Category_name.3, Category_name.4, Category_name.5 )) %>% 
  rename("Expenditure Category" = Category_name, "1 Year CAGR" = cagr_1, "2 Year CAGR" = cagr_2, "3 Year CAGR" = cagr_3, "5 Year CAGR" = cagr_5, "10 Year CAGR" = cagr_10,"24 Year CAGR" = cagr_25 )
...
# This works for one variable at a time
cagr_25 <- calc_cagr(rev_long, num_years) %>% 
     # group_by(Category) %>%
  summarize(cagr_25 = round(sum(cagr*100, na.rm = TRUE), 2))
...
CAGR_revenue_summary <- data.frame(cagr_1, cagr_2, cagr_3, cagr_5, cagr_10, cagr_24) %>%   
  select(-c(Category_name.1, Category_name.2, Category_name.3, Category_name.4, Category_name.5 )) %>% 
  rename("Revenue Category" = Category_name, "1 Year CAGR" = cagr_1, "2 Year CAGR" = cagr_2, "3 Year CAGR" = cagr_3, "5 Year CAGR" = cagr_5, "10 Year CAGR" = cagr_10,"24 Year CAGR" = cagr_25 )
...
expenditure_change_majorcats <- exp_long_majorcats %>%
  ...
  mutate("FY 2024 ($ Billions)" = round(Dollars_2024/1000, digits = 1),
         "FY 2022 ($ Billions)" = round(Dollars_2023/1000, digits = 1),
...
revenue_change_majorcats <- rev_long_majorcats %>%
  ...
  select(-c(Dollars_2024, Dollars_2023, `1 Year CAGR`:`25-Year CAGR`)) 
...
write.xlsx(dataset_names, file = 'summary_file_FY24_wTotals_Sept102024.xlsx')
```
